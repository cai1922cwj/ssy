...
# PythonAnywhere 上的数据库路径（根据你的配置修改）
# 如果使用 MySQL，请使用 MySQLdb 连接

# ===== 如果使用 SQLite（在 PythonAnywhere 控制台运行）=====
DB_PATH = os.path.expanduser("~") + "/entropy_food.db"  # 或者你的数据库路径
...
# DB_NAME = 'your-database$database-name'
...
def add_exercises_sqlite():
    ...
    added_count = 0
    for ex in MORE_EXERCISES:
        if ex["name"] not in existing_names:
            cursor.execute("""
                INSERT INTO exercises (name, category, met_value, description)
                VALUES (?, ?, ?, ?)
            """, (ex["name"], ex["category"], ex["met_value"], ex["description"]))
            added_count += 1
        else:
            print(f"已存在: {ex['name']}")
